...
var
  FMovieList: TFMovieList;

implementation

uses unit1,unit3, Math, StrUtils;

{$R *.dfm}

function TFMovieList.BooltoInt(x: boolean): Integer;
begin
  if x then result := 1 else Result := 0;
end;

procedure TFMovieList.btnFileClick(Sender: TObject);
begin
  if OpenDialog1.Execute then
    edtpath.Text := OpenDialog1.FileName;
end;

procedure TFMovieList.BtnBackupClick(Sender: TObject);
var i : integer;
begin
...
procedure TFMovieList.btnUpdateClick(Sender: TObject);
begin
...
procedure TFMovieList.btnExitClick(Sender: TObject);
begin
  Close;
end;

procedure TFMovieList.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;
